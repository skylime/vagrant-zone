        ...
        when 'list'
          ...
          datasets.each_with_index do |disk, index|
            ...
            zfssnapshots.reverse.each_with_index do |snapshot, snapindex|
              attributes = snapshot.gsub(/\s+/m, ' ').strip.split
              if snapindex.zero?
                puts format('%<snapmaxlength>8s %<usedmax>-s %<availmax>s %<refermax>s ', snapmaxlength: '#' , snapmaxlength,  usedmax: attributes[0],availmax: attributes[1],refermax: attributes[0])
