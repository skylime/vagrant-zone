  ...
  module ProviderZone
    ...
    class Driver
      ...
      def etherstubdelete(uii, opts, etherstub)
        ...
        puts "#{@pfexec} dladm show-etherstub | grep stub_#{vnic_name} | awk '{ print $1 }' "
        uii.info(I18n.t('vagrant_zones.creating_ethervnic') + vnic_name.to_s) if ether_configured == "stub_#{vnic_name}"
      ...
      def etherstubcreatehvnic(uii, opts, etherstub)
        ...
        shrtsubnet = "#{IPAddr.new(opts[:netmask].to_s).to_i.to_s(2).count('1')}"
        hvnic_name = "h_vnic_#{config.partition_id}_#{opts[:nic_number]}"
        uii.info(I18n.t('vagrant_zones.creating_etherhostvnic') + "#{hvnic_name}")
