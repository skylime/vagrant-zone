        ...
        when 'list'
          ...
          datasets.each_with_index do |disk, index|
            ...
            zfssnapshots.reverse.each_with_index do |snapshot, snapindex|
              attributes = snapshot.gsub(/\s+/m, ' ').strip.split
              if snapindex.zero?
                puts sprintf '%<snapmaxlength>8s  %-*s  %-*s  %-*s  %-*s  %-*s', '#',  snapmaxlength, attributes[0], usedmaxlength, attributes[1], availmaxlength, attributes[2], refermaxlength, attributes[3], pathmaxlength, attributes[4]
              else
