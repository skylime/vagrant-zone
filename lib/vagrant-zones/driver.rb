        ...
        when 'list'
          ...
          datasets.each_with_index do |disk, index|
            ...
            zfssnapshots.reverse.each_with_index do |snapshot, snapindex|
              attributes = snapshot.gsub(/\s+/m, ' ').strip.split
              if snapindex.zero?
                puts format('%<snapnum>8s %<snapmaxlength-usedmax>s %<availmax>s %<refermax>s ', snapnum: '#' ,usedmax: attributes[0],  snapmaxlength: snapmaxlength,   availmax: attributes[1],refermax: attributes[0])
